...
public class ProtocolUtil {
    ...
    public static void sendPacketBypassingVia(Player player, PacketWrapper<?> packet) {
        ...
        if (user == null) {
            if (!warnedBadPlugin) {
                ForcePackPaper.getInstance().getLogger().severe("You have a poorly coded plugin creating entirely fake players that are not specified as such/");
                ForcePackPaper.getInstance().getLogger().severe("Additionally, this message can only appear when the player exists in the Bukkit#getOnlinePlayers map.");
                ForcePackPaper.getInstance().getLogger().severe("This has a performance impact, including memory leaks, and must be fixed.");
